...
public final class CommandMinion {
    ...
    private static int spawn(CommandContext<CommandSourceStack> ctx, String forced) throws com.mojang.brigadier.exceptions.CommandSyntaxException {
        ...
        for (BodyPartLocation loc : BodyPartLocation.values()) {
            String mn = minion.getBodyPartName(loc);
            if (mn == null || mn.isEmpty()) continue;
            var id = BodyPartItemIds.inferredPartId(mn, loc);
            if (!BodyPartConfigManager.INSTANCE.get(id).map(BodypartDefinition::validated).orElse(false)) {
                ctx.getSource().sendFailure(Component.translatable("commands.necromancy.minion.unconfigured_part", id.toString()));
                return 0;
            }
        }
