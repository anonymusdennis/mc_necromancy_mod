...
public final class CraftingManagerSewing {
    ...
    /** Snapshot of every sewing recipe (for JEI and tooling). */
    public static List<SewingRecipe> getRecipes() {
        synchronized (RECIPES) {
            return List.copyOf(new ArrayList<>(RECIPES));
        }
    }

    public static void onCommonSetup(FMLCommonSetupEvent event) {
        // Adapters are constructed via RegistryNecromancyEntities.bootstrap().
        event.enqueueWork(() -> {
    ...
    public static ItemStack findMatching(Container craftMatrix, Level level) {
        for (SewingRecipe r : RECIPES) {
            if (r.matches(craftMatrix)) {
                ItemStack out = r.result.copy();
                // Bodypart sewing outputs require validated JSON (same gate as altar summon).
                return BodyPartConfigGate.allows(out) ? out : ItemStack.EMPTY;
            }
        }
